...
class PolygonDataFetcher:
    ...
    def get_batch_data_for_all_stocks(self, save_to_disk=True):
        """
        Lấy dữ liệu cho tất cả các cổ phiếu được cấu hình
        
        Args:
            save_to_disk (bool): Nếu True, tự động lưu dữ liệu vào đĩa
        ...
        if save_to_disk:
            ...
            self.save_market_data(results)
            
            # Lưu dữ liệu CSV cho từng cổ phiếu
            for symbol, data in results.items():
                if 'error' not in data:
                    if 'daily_data' in data and isinstance(data['daily_data'], pd.DataFrame):
                        self.save_daily_data_as_csv(symbol, data['daily_data'])
    ...
    def save_market_data(self, data, folder="data/raw", filename=None):
        """
        Lưu dữ liệu thị trường vào đĩa với định dạng chuẩn cho accuracy_evaluator
                ...
                for symbol, symbol_data in data.items():
                    ...
                    for key, value in symbol_data.items():
                        if isinstance(value, pd.DataFrame):
                            # Đối với DataFrame, chuyển thành dictionary với mỗi cột là một mảng
                            df_dict = {}
                            for column in value.columns:
                                df_dict[column] = value[column].tolist()
                            # Thêm cột index nếu nó là timestamp
                            if value.index.name == 'timestamp' or isinstance(value.index, pd.DatetimeIndex):
                                df_dict['timestamp'] = value.index.astype(str).tolist()
                            serializable_data[symbol][key] = df_dict
                        else:
                            serializable_data[symbol][key] = value
                
                json.dump(serializable_data, f, indent=2, default=str)
            
            logger.info(f"Đã lưu dữ liệu thị trường vào {file_path}")
            return file_path
        except Exception as e:
            logger.error(f"Lỗi khi lưu dữ liệu thị trường: {str(e)}")
            return None

    def save_daily_data_as_csv(self, symbol, data, folder="data/raw"):
        """
        Lưu dữ liệu ngày của một cổ phiếu vào file CSV
        
        Args:
            symbol (str): Mã cổ phiếu
            data (DataFrame): DataFrame chứa dữ liệu OHLCV
            folder (str): Thư mục lưu trữ
        ...
        # Tạo tên file với khoảng thời gian
        ...
        filename = f"{symbol}_daily_{start_date}_{end_date}.csv"
        file_path = os.path.join(folder, filename)
        
        try:
            # Kiểm tra và đảm bảo dữ liệu có định dạng phù hợp
            df_to_save = data.copy()
            
            # Nếu index là timestamp hoặc datetime, reset nó để thành cột
            if isinstance(df_to_save.index, pd.DatetimeIndex):
                ...
                if df_to_save.index.name == 'timestamp':
                    ...
                    df_to_save = df_to_save.rename(columns={'timestamp': 'date'})
            ...
            for col in required_columns:
                if col not in df_to_save.columns:
                    if col == 'volume':
                        df_to_save[col] = 0  # Giá trị mặc định cho volume
                    else:
                        logger.warning(f"Cột {col} không có trong dữ liệu, không thể lưu CSV")
                        return None
            
            # Lưu file
            df_to_save.to_csv(file_path, index=False)
            logger.info(f"Đã lưu dữ liệu ngày cho {symbol} vào {file_path}")
            return file_path
        except Exception as e:
            logger.error(f"Lỗi khi lưu dữ liệu ngày cho {symbol}: {str(e)}")
            logger.error(traceback.format_exc())
            return None

    # Thêm vào utils/data_fetcher.py
    def save_data_to_disk(self, data, filename):
        """
        Lưu dữ liệu thị trường vào đĩa
